...
Photo by [Soundtrap](https://unsplash.com/@soundtrap) on [Unsplash](https://unsplash.com)
...
5. **Automatic Speech Recognition** — this article (_Speech-to-Text algorithm and architecture, using CTC Loss and Decoding for aligning sequences._)
...
![Automatic Speech Recognition uses audio waves as input features and the text transcript as target labels (Image by Author)]({{ site.baseurl }}/assets/images/AudioASR/App-1.png)
...
![Transforming raw audio waves to spectrogram images for input to a deep learning model (Image by Author)]({{ site.baseurl }}/assets/images/AudioASR/Data-1.png)
...
![Spectrograms are processed by a convolutional network to produce feature maps (Image by Author)]({{ site.baseurl }}/assets/images/AudioASR/Model-1.png)
...
![Recurrent network processes frames from the feature maps (Image by Author)]({{ site.baseurl }}/assets/images/AudioASR/Model-2.png)
...
![Linear layer generates character probabilities for each timestep (Image by Author)]({{ site.baseurl }}/assets/images/AudioASR/Model-3.png)
...
![The model decodes the character probabilities to produce the final output (Image by Author)]({{ site.baseurl }}/assets/images/AudioASR/Model-4.png)
...
![On the left is the alignment we need. But how do we get it?? (Image by Author)]({{ site.baseurl }}/assets/images/AudioASR/Align-2.png)
...
![In reality, spoken speech is not neatly aligned for us (Image by Author)]({{ site.baseurl }}/assets/images/AudioASR/Align-1.png)
...
![The continuous audio is sliced into discrete frames and input to the RNN (Image by Author)]({{ site.baseurl }}/assets/images/AudioASR/CTC-3.png)
...
![CTC Decode algorithm (Image by Author)]({{ site.baseurl }}/assets/images/AudioASR/CTC-1.png)
...
![CTC Loss algorithm (Image by Author)]({{ site.baseurl }}/assets/images/AudioASR/CTC-2.png)
...
![Count the Insertions, Deletions, and Substitutions between the Transcript and the Prediction (Image by Author)]({{ site.baseurl }}/assets/images/AudioASR/Wer-1.png)

The metric formula is fairly straightforward. It is the percent of differences relative to the total number of words.

![Word Error Rate computation (Image by Author)]({{ site.baseurl }}/assets/images/AudioASR/Wer-2.png)
